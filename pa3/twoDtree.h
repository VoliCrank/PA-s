...
private:
  ...
  pair<int, int> findSplit(stats &s, pair<int, int> ul, pair<int, int> lr,
                           bool vert);

  void fillColor(PNG *out, Node *subRoot);
  Node *copy_helper(Node *subRoot, Node *orig);
  void clear_helper(Node *subRoot);
  void prune_helper(Node *subRoot, int tol);
  bool valid(Node *subRoot, RGBAPixel avg, int tol);
  void size_helper(Node *subRoot, int tol, RGBAPixel avg);
  int ideal_helper(int lower, int upper,int leaves);
};
