 ...
 * the fill, then it should be colored with the given color. Otherwise, its
 *color should not change.
 *
 * The following criteria should be used to determine if a position p is near
 * a border: if p is within distance `borderSize` of any cell that will be
 *outside the fill, then the pixel at position p should be colored with the
 *given color.
 *
...
public:
   ...
   */
  borderColorPicker(unsigned int borderSize, HSLAPixel fillColor, PNG &img,
                    double tolerance);
...
private:
  bool checkBoundaryPixels(point p);
  bool checkUpDown(point p, int i, int j);
  bool checkOne(int x, int y, HSLAPixel ctr) ;
